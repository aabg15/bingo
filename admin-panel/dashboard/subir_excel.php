...
if (!empty($archivotmp)) {
  ...
  foreach ($lineas as $linea) { //para no leer la primera fila
    ...
    if ($i != 0) {
      ...
      $departamento          = !empty($datos[2])  ? ($datos[2]) : '';
      $ciudad                = !empty($datos[3])  ? ($datos[3]) : '';
      ...
      $direccion             = !empty($datos[16])  ? ($datos[16]) : '';
      $distrito              = !empty($datos[17])  ? ($datos[17]) : '';
      $sede                  = !empty($datos[18])  ? ($datos[18]) : '';
      $telefono              = !empty($datos[19])  ? ($datos[19]) : '';
      $nombre                = !empty($datos[20])  ? ($datos[20]) : '';
      $apellidos             = !empty($datos[21])  ? ($datos[21]) : '';
      ...
      if (!empty($dni)) {
        ...
        $SqlCanJugada = ("SELECT cant_jugada from cliente where dni='" . $dni . "' ");
        ...
        //$cant_obtenida = mysqli_rows($cant_obtenida);
        //echo $cant_obtenida;
        //echo gettype($cant_jugadas);

        $cant_obtenida = intval($cant_obtenida);
        $cant_jugadas = intval($cant_jugadas);
        $masoportunidades = $cant_obtenida + $cant_jugadas;

        $Sqlaumentar = "UPDATE `cliente` SET `cant_jugada`='$masoportunidades' where `dni`='$dni';";
